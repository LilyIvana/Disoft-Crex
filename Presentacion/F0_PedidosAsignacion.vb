...
#End Region
    ...
    Private Sub ANULARPEDIDOToolStripMenuItem1_Click(sender As Object, e As EventArgs) Handles ANULARPEDIDOToolStripMenuItem1.Click
        Try
            ...
            Dim dtVerificar As DataTable = L_VerificarPedido(codPedido.ToString)
            If dtVerificar.Rows.Count > 0 Then

                If dtVerificar.Rows(0).Item("oacnconc") > 0 Then
                    P_prAnularFacturaVenta(codPedido)

                ElseIf dtVerificar.Rows(0).Item("oacnconc") = 0 And dtVerificar.Rows(0).Item("oaccbnumi") = 4 Then '4 porque ese es el repartidor por defecto para venta/cierres directos 
                    P_prAnularFacturaVenta(codPedido)

                Else
                    ToastNotification.Show(Me, " No se puede anular el Pedido: ".ToUpper + codPedido.ToString + " porque no se grabó el Movimiento Chofer Salida".ToUpper,
                                           My.Resources.WARNING, 4700, eToastGlowColor.Green, eToastPosition.TopCenter)
                    Exit Sub
                End If
            ...
            Else
                'Anulación cuando el pedido no ha sido asignado a ningún chofer o se ha quitado la asignación por algún motivo

                If (MessageBox.Show("Esta seguro de ANULAR el Pedido: " + Convert.ToString(codPedido) + "?", "PREGUNTA", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes) Then
    ...
    Private Sub P_prAnularFacturaVenta(codPedido As Integer)
        Dim img As Bitmap = New Bitmap(My.Resources.WARNING, 50, 50)
